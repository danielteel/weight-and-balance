...
export default function Aircraft({aircraft, aircraftDispatch}){
    ...
    const newAircraftAction = () => {
        aircraftDispatch('create', null);
    }
                ...
                <Table.Header>
                    <Table.Row>
                        <Table.HeaderCell colSpan='5'>
                            <Button floated='left' icon='add' labelPosition='left' positive size='small' content='New Aircraft'  onClick={newAircraftAction}/>
                        </Table.HeaderCell>
                    </Table.Row>
                    <Table.Row>
                        ...
                        aircraft?.length ?
                            aircraft.map( ac => (
                                <Table.Row>
                                    ...
                                    <Table.Cell style={noPadCell}>
                                        <TouchInput as={Input} value={ac.weight} onChange={(v)=>aircraftDispatch('update', {...ac, weight: v})} fluid  type='number' title='Moment' input={inputStyle}/>
                                    </Table.Cell>
                                    <Table.Cell style={noPadCell}>
